...
def registerView(request):
    ...
    serializer.is_valid(raise_exception=True)
    try:
        mergin.MerginClient(login=request.data["username"], password=request.data["password"])
...
def updateUserView(request):
    print(request)
    try:
        user = models.Account.objects.get(id = request.session["id"])
    ...
    # Pass in user instance and request data to the serializer
    serializer = serializers.UserUpdateSerializer(user, data=request.data, partial=True)

    # Validate data and update the user if valid
